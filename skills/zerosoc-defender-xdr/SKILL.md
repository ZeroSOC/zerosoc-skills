...
| **identity.directory** — who is this account? | the directory record behind the identity workload | an account outside the directory (a local account, a guest that never signed in) has no record; the alert may still name it |
...
| **reputation.multi_engine** — what is this address or URL? | the vendor's reputation for it | one engine, not a consensus: the class is named for what the *method* wants, and this source answers the narrower question. Where a Finding turns on reputation alone, say whose |
...
- **`classification` and `determination` are the source's verdict, and may be a human's or the product's.** An automated investigation writes them exactly as an analyst does, and the record does not always say which. They are recorded on the extension object and weighed by nobody: the Case's verdict is the executor's.
