...
words. A host composes this text with the Case's data; it does not restate it.
...
- Tag each result Malicious or Benign at Low, Medium or High as the playbook prescribes; a result
  that bears on neither side is context with confidence null.
...
- A Benign finding names the playbook condition that applies: `false_positive` when the detection
  misfired, `benign` when authorized activity legitimately matched it.
