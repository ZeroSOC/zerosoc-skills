...
def _malicious(observation):
    tag = observation.get("tag") if isinstance(observation.get("tag"), dict) else {}
    side = observation.get("side", tag.get("side"))
    return str(side or "").strip().lower() == "malicious" and observation.get("retracted") is not True
...
def build(ledger):
    ...
    held = {}
    for name in ("observations", "entries"):
    ...
    observations, others = held["observations"], held["entries"]

    for observation in observations:
        when = parse_time(observation.get("first_seen"))
        label = observation.get("id", "?")
        if observation.get("first_seen") not in (None, "") and when is None:
            problems.append(f"observation {label}: first_seen {observation['first_seen']!r} is not a time this can read")
        moves_t0 = _malicious(observation) and when is not None
        if moves_t0:
    ...
    for entry in others:
        ...
        if moves_t0:
            malicious.append(when)  # an Alert is the first Malicious observation of its Case
...
def check(built):
    ...
    if built["t0"] is not None and not flagged:
        failures.append(f"T0 is {built['t0_utc']} and no entry carries it: the Note would state a first "
                        "malicious event the timeline does not show — flag the observation it comes from")
    if built.get("stated_matches") is False and built["derived_t0"] < built["stated_t0"]:
        failures.append(f"a Malicious entry at {_utc(built['derived_t0'])} happens before T0 ({_utc(built['stated_t0'])}): "
                        ...
                        "Context may come before T0; a Malicious observation may not")
        ...
        failures.append(f"the Case states T0 {_utc(built['stated_t0'])} and nothing Malicious is known at any time: "
                        "T0 is the earliest Malicious event, so it rests on an observation the ledger does not hold")
    elif built.get("stated_matches") is False:
        failures.append(f"the Case states T0 {_utc(built['stated_t0'])} and its earliest Malicious observation is at "
