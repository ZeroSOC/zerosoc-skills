...
class TheMethodSurface(unittest.TestCase):
    ...
    def test_a_observation_with_a_side_and_no_confidence_fails(self):
        note = self.note()
        note["observations"][0]["tag"] = {"side": "malicious", "confidence_id": None}

        self.assertTrue(any("no confidence" in f for f in note_elements.check(note, root=self.ELEMENTS_ROOT)))

    def test_a_observation_that_cites_no_event_fails(self):
        note = self.note()
        note["observations"][0]["event_refs"] = []
    ...
    def test_a_recommended_action_nobody_acted_on_is_not_a_failure(self):
        ...
        is how a Case of three alerts produced a Note of sixty-seven observations."""
        note = self.note()
    ...
    def note(self):
        return {
            ...
            "observations": [
                {"n": 1, "observation": "T1486 (Data Encrypted for Impact) was observed",
...
def ledger_note(**changes):
    """A Note as the skills' own ledgers hold it: observations with side, confidence and the events they
    ...
    note = {
        ...
        "rationale": "No Benign observation covers DF-1, so the coverage rule promotes the Case.",
...
class TheNoteCheck(unittest.TestCase):
    ...
    def test_however_a_note_writes_a_technique_code_it_is_conformant(self):
        for where, note in (("observation", ledger_note()), ("summary", ledger_note(summary="T1486 was seen on ws-01")),
                            ("rationale", ledger_note(rationale="T1114.003 stands uncovered"))):
            if where == "observation":
                note["observations"][0]["desc"] = "T1114.003 observed"
    ...
    def test_a_side_needs_a_confidence_the_framework_has(self):
        for confidence in (None, 0, "Unknown", "certain"):
            note = ledger_note()
            note["observations"][0]["confidence"] = confidence
            with self.subTest(confidence=confidence):
                self.assertTrue(any("no confidence" in f for f in self.failures(note)))

    def test_context_is_a_observation_with_no_side_and_carries_no_confidence(self):
        note = ledger_note()
        note["observations"][1]["confidence"] = "Medium"
    ...
    def test_the_observations_element_says_what_an_alert_observation_renders(self):
        """The disposition of a recommended action is no longer among them: what the executor ran
        renders as the Observation it produced, and what it did not run renders nowhere."""
        observations = next(e for e in note_elements.elements("triage", FRAMEWORK) if e["element"] == "observations")
        for words in ("ID (Name)", "remediation state", "recommended action the executor ran"):
            self.assertIn(words, observations["renders"])
        self.assertNotIn("disposition of the recommended actions", observations["renders"])

    def test_something_that_is_not_a_note_is_a_failure_and_not_a_traceback(self):
        for broken in ([], {"observations": "F1"}, ledger_note(observations=[{"id": "F1", "tag": "Malicious (High)"}])):
    ...
    def test_malformed_detection_metadata_is_a_failure_and_never_a_traceback(self):
        ...
        note = ledger_note()
        note["observations"][0]["event_refs"] = 12345
    ...
    def test_an_investigation_ledger_holds_its_alerts_among_its_observations(self):
        note = ledger_note(kind="investigation", timeline=[{"id": "F1"}])
        del note["alerts"], note["detection_metadata"]
        note["observations"][0].update(alert_type="Malware / loader execution", entity="ws-01")
    ...
    def test_a_case_closed_on_its_alerts_alone_renders_them_as_its_observations(self):
        self.assertEqual(self.failures(ledger_note(observations=[])), [], "the Alerts are the Case's first Observations")
        empty = ledger_note(observations=[], alerts=[])
        del empty["detection_metadata"]
        self.assertTrue(any("Observations" in f for f in self.failures(empty)))
    ...
    def test_the_older_tag_shape_is_still_read(self):
        note = ledger_note(observations=[{"n": 1, "observation": "T1486 (Data Encrypted for Impact) was observed",
                                      "tag": {"side": "malicious", "confidence_id": 3}, "event_refs": ["A1"]}])
        self.assertEqual(self.failures(note), [])
...
class AFrameworkChangeIsASkillsReleaseAndNothingElse(unittest.TestCase):
    ...
    def test_a_renamed_and_an_added_element_are_required_with_no_script_change(self):
        def rewrite(text):
            ...
            rest = rest.replace("8.  **Provenance** —", "8.  **Provenance and Custody** —", 1)
            rest = rest.replace("\n**Two former elements", "9.  **Lessons** — what this Case teaches the next one.\n\n**Two former elements", 1)
    ...
    def test_prose_between_two_elements_does_not_end_the_list(self):
        def rewrite(text):
            return text.replace("    Each **Alert** Observation also renders", "Each **Alert** Observation also renders", 1)
    ...
    def test_a_list_inside_an_element_is_not_a_list_of_elements(self):
        def rewrite(text):
            return text.replace("3.  **Rationale** —", "    1. **Alerts** — first\n    2. **Checks** — then\n3.  **Rationale** —", 1)
        names = [e["element"] for e in note_elements.elements("triage", self.served(rewrite))]
    ...
    def test_two_elements_one_field_or_a_name_with_no_field_stops_the_script(self):
        for change in (("8.  **Provenance** —", "8.  **Summary** —"), ("8.  **Provenance** —", "8.  **来歴** —"),
                       ("8.  **Provenance** —", "8.  **Observations!** —")):
    ...
    def test_none_quoted_about_something_else_does_not_make_an_element_optional(self):
        def rewrite(text):
            return text.replace("    Each **Alert** Observation also renders", "    An Alert with no technique renders \"None.\" there. Each **Alert** Observation also renders", 1)
        observations = next(e for e in note_elements.elements("triage", self.served(rewrite)) if e["element"] == "observations")
        self.assertFalse(observations["may_be_empty"])
    ...
    def test_every_element_of_both_notes_is_read_at_this_pin(self):
        self.assertEqual([e["element"] for e in note_elements.elements("triage", FRAMEWORK)],
                         ["summary", "classification", "rationale", "observations", "timeline",
                          "response_actions", "visibility_gaps", "provenance"])
        self.assertEqual([e["element"] for e in note_elements.elements("investigation", FRAMEWORK)],
                         ["summary", "classification", "rationale", "observations", "reclassification_pivots",
                          "timeline", "response_actions", "visibility_gaps", "provenance"])


class TheCaseTimeline(unittest.TestCase):
    def ledger(self, *observations, **more):
        return dict({"now": "2026-09-14T15:12:00Z", "observations": list(observations)}, **more)

    def observation(self, fid, first_seen, side="Malicious", **more):
        return dict({"id": fid, "desc": f"observation {fid}", "side": side, "confidence": "High" if side else None,
                     "first_seen": first_seen, "timeline": True, "event_refs": [f"EVT-{fid}"]}, **more)

    def test_t0_is_derived_from_the_earliest_malicious_observation(self):
        built = timeline.build(self.ledger(
            self.observation("F2", "2026-09-14T14:58:00Z"), self.observation("F1", "2026-09-14T14:55:41Z"),
            self.observation("F0", "2026-09-14T14:40:00Z", side=None)))
    ...
    def test_only_the_observations_flagged_for_the_narrative_are_entries(self):
        built = timeline.build(self.ledger(self.observation("F1", "2026-09-14T14:55:41Z"),
                                           self.observation("F2", "2026-09-14T14:56:00Z", timeline=False)))
        self.assertEqual([e["id"] for e in built["entries"]], ["F1"])

    def test_a_retracted_observation_moves_nothing(self):
        built = timeline.build(self.ledger(self.observation("F0", "2026-09-14T13:00:00Z", retracted=True),
                                           self.observation("F1", "2026-09-14T14:55:41Z")))
        self.assertEqual(built["t0_utc"], "2026-09-14T14:55:41Z")

    def test_the_earliest_malicious_observation_left_off_the_timeline_is_a_failure(self):
        built = timeline.build(self.ledger(self.observation("F1", "2026-09-14T14:55:41Z", timeline=False),
                                           self.observation("F2", "2026-09-14T14:58:00Z")))
        self.assertTrue(any("no entry carries it" in f for f in timeline.check(built)))

    def test_a_stated_t0_that_is_not_the_earliest_malicious_observation_is_a_failure(self):
        built = timeline.build(self.ledger(self.observation("F1", "2026-09-14T14:55:41Z"), t0="2026-09-14T14:58:00Z"))
    ...
    def test_the_same_instant_is_the_same_instant_however_it_is_written(self):
        built = timeline.build(self.ledger(self.observation("F1", "2026-09-14T16:55:41+02:00"),
                                           self.observation("F2", "2026-09-14T14:55:41.5000000Z"),
    ...
    def test_a_time_nobody_can_read_is_a_failure_and_not_a_traceback(self):
        built = timeline.build(self.ledger(self.observation("F1", "yesterday, around three")))
        self.assertTrue(any("F1" in f for f in timeline.check(built)))
        self.assertTrue(timeline.check(timeline.build({"entries": "F1"})))

    def test_a_observation_is_placed_by_when_the_thing_happened_never_by_when_it_was_made(self):
        found = self.observation("F1", None, at="2026-09-14T15:03:10Z")
    ...
    def test_t0_is_carried_by_a_malicious_entry_never_by_whatever_shares_its_instant(self):
        ...
        for label, other in (("benign", self.observation("B1", at, side="Benign")),
                             ("context", self.observation("C1", "2026-09-14T14:55:40.500Z", side=None)),
                             ("retracted", self.observation("R1", at, retracted=True))):
            built = timeline.build(self.ledger(other, self.observation("M1", at)))
            with self.subTest(sharing=label):
                self.assertEqual([e["id"] for e in built["entries"] if e["is_t0"]], ["M1"])
        handover = timeline.build(self.ledger(self.observation("M1", "2026-09-14T15:12:00Z"),
    ...
    def test_a_benign_entry_at_t0_does_not_hide_that_the_malicious_one_is_off_the_timeline(self):
        at = "2026-09-14T14:55:41Z"
        built = timeline.build(self.ledger(self.observation("M1", at, timeline=False), self.observation("B1", at, side="Benign")))
        self.assertTrue(any("no entry carries it" in f for f in timeline.check(built)))

    def test_a_stated_t0_with_nothing_malicious_known_is_a_failure(self):
        built = timeline.build(self.ledger(self.observation("B1", "2026-09-14T14:55:41Z", side="Benign"), t0="2026-09-14T14:55:41Z"))
        self.assertTrue(any("nothing Malicious" in f for f in timeline.check(built)))

    def test_a_malicious_observation_within_the_same_second_but_before_a_stated_t0_is_said(self):
        built = timeline.build(self.ledger(self.observation("M1", "2026-09-14T14:55:40.100Z"), t0="2026-09-14T14:55:41Z"))
        self.assertTrue(any("happens before T0" in f for f in timeline.check(built)))
        same_second = timeline.build(self.ledger(self.observation("M1", "2026-09-14T14:55:41.900Z"), t0="2026-09-14T14:55:41Z"))
    ...
    def test_a_time_out_of_any_case_is_unreadable_and_seconds_are_not_milliseconds(self):
        for bad in (1789397741, 1e400, float("nan"), 10 ** 18, "9999-12-31T23:59:59-05:00", -5):
            built = timeline.build(self.ledger(self.observation("F1", bad)))
            with self.subTest(first_seen=str(bad)):
                self.assertTrue(any("F1" in f for f in timeline.check(built)))

    def test_what_is_not_a_observation_or_a_readable_now_is_said(self):
        built = timeline.build({"now": "soon", "observations": ["F1", 3]})
        found = " | ".join(timeline.check(built))
        self.assertIn("now", found)
        self.assertIn("observations", found)
    ...
    def test_it_is_t0_only_on_a_confirmed_incident(self):
        ...
        open_case = timeline.build(self.ledger(self.observation("M1", "2026-09-14T14:55:41Z")))
        self.assertEqual((open_case["start_time_utc"], open_case["confirmed_incident"]), ("2026-09-14T14:55:41Z", False))
        confirmed = timeline.build(self.ledger(self.observation("M1", "2026-09-14T14:55:41Z"), recorded_verdict="True Positive"))
        self.assertTrue(confirmed["confirmed_incident"])

    def test_the_older_tag_shape_moves_t0_too(self):
        built = timeline.build(self.ledger({"n": 1, "observation": "ransomware behaviour", "tag": {"side": "malicious", "confidence_id": 3},
...
class FromTheCommandLine(unittest.TestCase):
    ...
    def test_a_note_that_is_not_conformant_exits_1_and_says_why(self):
        ...
        document["observations"][0]["event_refs"] = []
        ...
        self.assertIn("FAIL: observation F1: cites no event", ran.stderr)
...
class ARecommendationIsCarriedAndNotAccountedFor(unittest.TestCase):
    ...
    back: an executor that ran one writes an Observation, and the Observation is what is checked."""
